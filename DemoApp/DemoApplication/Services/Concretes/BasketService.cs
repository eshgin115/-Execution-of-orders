            ...
            List<ProductCookieViewModel> AddToCookie()
            {
                ...
                if (productCookieViewModel is null)
                {
                    productsCookieViewModel
                        !.Add(new ProductCookieViewModel(book.Id, book.Title, book.BookImages.Take(1)!.FirstOrDefault()! != null
                        ? _fileService.GetFileUrl(book.BookImages!.Take(1)!.FirstOrDefault()!.ImageNameInFileSystem!, UploadDirectory.Book) : string.Empty , 1, book.Price, book.Price));
                }
                else
                {
