...
namespace DemoApplication.Areas.Admin.Controllers
{
    ...
    public class BookImageController : Controller
    {
        ...
        public async Task<IActionResult> UpdateAsync(int bookId, int bookImageId)
        {
            ...
            {
                ImageUrL = _fileService.GetFileUrl(bookImage.ImageNameInFileSystem, UploadDirectory.Book),
                Order = bookImage.Order,
            };

            return View(model);
        }
        [HttpPost("{bookId}/image/{bookImageId}/update", Name = "admin-book-image-update")]
        public async Task<IActionResult> UpdateAsync(int bookId, int bookImageId, [FromForm] UpdateViewModel model)
        {
            var bookImage = await _dataContext.BookImages
                .FirstOrDefaultAsync(bi => bi.Id == bookImageId && bi.BookId == bookId);
            ...
            if (bookImage is null) return NotFound();
            ...
            if (!(model.Image == null))
            {
                ...
                var imageNameInSystem = await _fileService.UploadAsync(model.Image, UploadDirectory.Book);
                bookImage.ImageNameInFileSystem = imageNameInSystem;
                bookImage.ImageName = model.Image.FileName;
            }
            ...
            return RedirectToRoute("admin-book-image-update", new { bookId = bookId });
        }
